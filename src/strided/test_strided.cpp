...
void rowmajor_spxd(const double* A, const double* B, double* C) {
  __asm__ __volatile__(
    ...
    "vmovapd 0(%%rdx), %%zmm25\r\n"         // C[0,:]
    "vmovapd 64(%%rdx), %%zmm26\r\n"        // C[1,:]
    "vmovapd 128(%%rdx), %%zmm27\r\n"       // C[2,:]
    "vmovapd 192(%%rdx), %%zmm28\r\n"       // C[3,:]
    "vmovapd 256(%%rdx), %%zmm29\r\n"       // C[4,:]
    "vmovapd 320(%%rdx), %%zmm30\r\n"       // C[5,:]
    "vmovapd 384(%%rdx), %%zmm31\r\n"       // C[6,:]
    "vmovapd 448(%%rdx), %%zmm32\r\n"       // C[7,:]
    ...
    "vfmadd231pd 0(%%rdi)%{1to8%}, %%zmm0, %%zmm25\r\n"       // C[0,:] += A[0,0] .* B[0,:]
    "vfmadd231pd 8(%%rdi)%{1to8%}, %%zmm0, %%zmm32\r\n"       // C[7,:] += A[7,0] .* B[0,:]

    // C[1,:] += broadcast(A[ri,1]) * B[1,:] for ri in range(r) if A[ri,1] != 0
    "vfmadd231pd 16(%%rdi)%{1to8%}, %%zmm1, %%zmm26\r\n"       // C[1,:] += A[1,1] * B[1,:]
    "vfmadd231pd 24(%%rdi)%{1to8%}, %%zmm1, %%zmm28\r\n"       // C[3,:] += A[3,1] * B[1,:]

    // C[2,:] += broadcast(A[ri,2]) * B[2,:] for ri in range(r) if A[ri,2] != 0
    "vfmadd231pd 32(%%rdi)%{1to8%}, %%zmm2, %%zmm27\r\n"       // C[2,:] += A[2,2] * B[2,:]
    "vfmadd231pd 40(%%rdi)%{1to8%}, %%zmm2, %%zmm32\r\n"       // C[7,:] += A[7,2] * B[2,:]

    // C[3,:] += broadcast(A[ri,3]) * B[3,:] for ri in range(r) if A[ri,3] != 0
    "vfmadd231pd 48(%%rdi)%{1to8%}, %%zmm3, %%zmm26\r\n"       // C[1,:] += A[1,3] * B[3,:]
    "vfmadd231pd 56(%%rdi)%{1to8%}, %%zmm3, %%zmm28\r\n"       // C[3,:] += A[3,3] * B[3,:]

    // C[4,:] += broadcast(A[ri,4]) * B[4,:] for ri in range(r) if A[ri,4] != 0
    "vfmadd231pd 64(%%rdi)%{1to8%}, %%zmm4, %%zmm29\r\n"       // C[4,:] += A[4,4] * B[4,:]
    "vfmadd231pd 72(%%rdi)%{1to8%}, %%zmm4, %%zmm32\r\n"       // C[7,:] += A[7,4] * B[4,:]

    // C[5,:] += broadcast(A[ri,5]) * B[5,:] for ri in range(r) if A[ri,5] != 0
    "vfmadd231pd 80(%%rdi)%{1to8%}, %%zmm5, %%zmm30\r\n"       // C[5,:] += A[5,5] * B[5,:]

    // C[6,:] += broadcast(A[ri,6]) * B[6,:] for ri in range(r) if A[ri,6] != 0
    "vfmadd231pd 88(%%rdi)%{1to8%}, %%zmm6, %%zmm31\r\n"       // C[6,:] += A[6,6] * B[6,:]

    // C[7,:] += broadcast(A[ri,7]) * B[7,:] for ri in range(r) if A[ri,7] != 0
    "vfmadd231pd 96(%%rdi)%{1to8%}, %%zmm7, %%zmm25\r\n"       // C[0,:] += A[0,7] * B[7,:]
    "vfmadd231pd 104(%%rdi)%{1to8%}, %%zmm7, %%zmm27\r\n"      // C[2,:] += A[2,7] * B[7,:]
    "vfmadd231pd 112(%%rdi)%{1to8%}, %%zmm7, %%zmm29\r\n"      // C[4,:] += A[4,7] * B[7,:]
    "vfmadd231pd 120(%%rdi)%{1to8%}, %%zmm7, %%zmm32\r\n"      // C[7,:] += A[7,7] * B[7,:]

    // Scatter entire C-block
    "vmovapd %%zmm25, 0(%%rdx)\r\n"         // C[0,:]
    "vmovapd %%zmm26, 64(%%rdx)\r\n"        // C[1,:]
    "vmovapd %%zmm27, 128(%%rdx)\r\n"       // C[2,:]
    "vmovapd %%zmm28, 192(%%rdx)\r\n"       // C[3,:]
    "vmovapd %%zmm29, 256(%%rdx)\r\n"       // C[4,:]
    "vmovapd %%zmm30, 320(%%rdx)\r\n"       // C[5,:]
    "vmovapd %%zmm31, 384(%%rdx)\r\n"       // C[6,:]
    "vmovapd %%zmm32, 448(%%rdx)\r\n"       // C[7,:]
