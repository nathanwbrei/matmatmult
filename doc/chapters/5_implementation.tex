...
The syntax tree is built up in stages of increasing complexity. All AST nodes inherit from the abstract base class \py{AsmStmt}, which enforces the existence of an \py{accept()} method, a \py{comment} string, and nothing else. A limited attempt was made to provide type information, which ultimately consisted of . 
...
\begin{description}
  ...
  \item [Compare] Wraps the \py{cmp} instruction
  \item [Jump to label] 
  \item [Declare label] \py{DeclLabelStmt} inserts a label into the assembly
  \item [Declare data] \py{DeclDataStmt} pastes a \py{Label} or a \py{Constant} directly into the code segment, essentially wrapping the \py{db,dw,dd,dq} instructions. It is useful for creating jump tables or constant arrays. It could be modified to target other segments as well.
\end{description}
