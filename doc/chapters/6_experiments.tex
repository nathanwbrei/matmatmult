...
By averaging the slopes of each line, the jump penalty can be quickly estimated at around 0.2 microseconds/jump. (Recall that in this case, a `jump' consists of a direct jump to a MicroSparse kernel, followed by an indirect jump from a register back to the calling location.) Though the penalty does become slightly higher for larger numbers of nonzeros, the two variables are close enough to be approximated as independent. This plot also suggests that the jumps are not hurting our pipeline performance. If it did, we would expect the penalty to be steeper for lower $nnz$ instead. 
