...
The UnrolledSparse approach is fundamentally unable to scale beyond a certain number of nonzeros, and the GeneralSparse approach seeks to ameliorate this by reintroducing some limited control flow and filling in zero entries. The extent of the fill-in depends heavily on the structure of the original sparsity pattern. On the other hand, the time to solution is expected to increase linearly with the number of jumps. We would like examine how this \emph{jump penalty} varies with the number of nonzeros.


with each jumps is expected to be linear with the number of jumps. Thus we can measure a jump penalty  Because the number of jumps is equal to the \emph{total} number of nonempty blocks, the matrix size, 


This becomes tricky to predict because it is coupled to the matrix and block sizes.


  This experiment explores how the performance is affected by the total number of jumps, and how that varies with number of nonzeros and matrix size. 

First we examine how the performance varies according to the number of jumps.  


explores the effect of increasing the number of jumps while keeping the matrix size constant. This is repeated for several choices of $nnz$ in order to see how . This is achieved by taking a matrix with a random sparsity pattern and repeatedly partitioning it with 

Because GeneralSparse currently delegates to MicroSparse instead of UnrolledSparse, the maximum block sizes are limited, 
